...
; 0000 009A #asm("sei")
	sei
; 0000 009B Dig_init(); //инициализация массива с двоичным кодом
	RCALL _Dig_init
; 0000 009C while (1)
_0xB:
; 0000 009D       {///// Условия часов.
; 0000 009E        if(sek==60)          // если сек = 60
	LDI  R30,LOW(60)
	CP   R30,R5
	BRNE _0xE
; 0000 009F        {
; 0000 00A0        min++;              // добавляем 1 к переменной "минута"
	INC  R4
; 0000 00A1        sek=0;              // зануляем переменную "секунда"
	CLR  R5
; 0000 00A2        }
; 0000 00A3        if(min==60)         // если мин = 60
_0xE:
	LDI  R30,LOW(60)
	CP   R30,R4
	BRNE _0xF
; 0000 00A4        {
; 0000 00A5        hour++;             // добавляем 1 к переменной "час"
	INC  R7
; 0000 00A6        min=0;              // зануляем переменную "минута"
	CLR  R4
; 0000 00A7        }
; 0000 00A8        if (hour==24)        // так как у нас часы имеют 24 часовый формат
_0xF:
	LDI  R30,LOW(24)
	CP   R30,R7
	BRNE _0x10
; 0000 00A9        {                    // при достыжении 24 часов, онулируем все переменные.
; 0000 00AA        hour=0;
	CLR  R7
; 0000 00AB        min=0;
	CLR  R4
; 0000 00AC        sek=0;
	CLR  R5
; 0000 00AD        }
; 0000 00AE          // защита переполнения переменной, она нужна для того чтобы в моментах когда время не переходило в минус.
...
; 0000 00B2   min=0;
	CLR  R4
; 0000 00B3          // выводим переменные (стробирование)
; 0000 00B4         Display(hour); //разложили "часы" на 2 цифры и отобразили по очереди
_0x12:
	MOV  R30,R7
	RCALL SUBOPT_0x2
; 0000 00B5          PORTB=254;    //даем лог 0 для катода 1 разряда
	LDI  R30,LOW(254)
	RCALL SUBOPT_0x3
; 0000 00B6          PORTD=Disp6;  //1 цифра
; 0000 00B7         delay_ms(5);
; 0000 00B8          PORTB=253;    //даем лог 0 для катода 2 разряда
	LDI  R30,LOW(253)
	RCALL SUBOPT_0x4
; 0000 00B9         PORTD=Disp7;   //2 цифра
; 0000 00BA         delay_ms(5);
; 0000 00BB          Display(min);  //разложили "минуты" на 2 цифры и отобразили по очереди
	MOV  R30,R4
	RCALL SUBOPT_0x2
; 0000 00BC         PORTB=251;      //даем лог 0 для катода 3 разряда
	LDI  R30,LOW(251)
	RCALL SUBOPT_0x3
; 0000 00BD         PORTD=Disp6;  //3 цифра
; 0000 00BE         delay_ms(5);
; 0000 00BF          PORTB=247;    //даем лог 0 для катода 4 разряда
	LDI  R30,LOW(247)
	RCALL SUBOPT_0x4
