...
static int uploadData(char *dataBuffer, int &startAddr, int &endAddr) {
 ...
 if((err = usbOpenDevice(&dev, IDENT_VENDOR_NUM, IDENT_VENDOR_STRING,
  ...
  printf("Error opening HIDBoot device: %s\n", usbErrorMessage(err));
 ...
 if((err = usbGetReport(dev, USB_HID_REPORT_TYPE_FEATURE, 1, buffer.bytes, &len)) != 0){
  printf("Error reading page size: %s\n", usbErrorMessage(err));
  goto errorOccurred;
 }
 if(len < sizeof(buffer.info)){
  printf("Not enough bytes in device info report (%d instead of %d)\n",
 ...
 if(endAddr > deviceSize - 2048){
  printf("Data (%d bytes) exceeds remaining flash size!\n", endAddr);
  err = -1;
  goto errorOccurred;
 }
 ...
 while(startAddr < endAddr){
  ...
  if((err = usbSetReport(dev, USB_HID_REPORT_TYPE_FEATURE, buffer.bytes, sizeof(buffer.data))) != 0){
   printf(" Error uploading data block: %s\n", usbErrorMessage(err));
   goto errorOccurred;
  }
...
int _cdecl main(int argc, char **argv) {
 ...
 for (int i=0; i<10; i++) {
  if(!uploadData(dataBuffer, startAddress, endAddress)) {
   printf("The firmware is written successfully after %i retries.\n",i);
   return 0;
  }
 }
 return 1;
