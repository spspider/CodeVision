 ...
 *[16] MISO  4 = /INI -  C2 (16)
 *[17] SCK   5 = /SEL - /C3 (17)
 * [8] XTAL2 6 = Quarz, nunmehr frei (wird per internem Pullup high gezogen...)
 * [7] XTAL1 7 = Quarz, nunmehr frei (...um erhöhter Stromaufnahme vorzubeugen)
 ...
 * Aber die 250 Leiterplatten sind gefertigt...
 *
...
static void out2(uchar b) {
 ...
 b <<= 2;
 PORTB = b | 0xC0;
 if (Feature & 0x04) DDRB = ~b & 0x3C;	// USB-DDR-Bits müssen 0 bleiben
}
...
static uchar ProcessInOut(const uchar *data, uchar len, uchar *buf) {
 ...
 do{
  ...
  //case 0x21: if (!test(value)) goto exitloop;
   }
  }
 }while (--len);
...
uchar lastTimer0Value;	// erforderlich in "libs-device/osctune.h"
...
int main(void) {
 ...
 { uchar osccal = eeprom_read_byte((uchar*)0xFFFA);
   if (osccal && osccal!=0xFF) OSCCAL = osccal;
 ...
 for(;;) {		// Hauptschleife
  ...
  if (SofCmp != t) {	// SOF eingetroffen?
   ...
   if (eeprom_is_ready()) {	// Feature-Byte im EEPROM nachführen
    if (eeprom_read_byte((uchar*)0xFFFB) != Feature)
