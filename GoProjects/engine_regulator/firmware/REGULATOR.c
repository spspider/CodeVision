...
TCNT2=0xC2;
//каждому порту установленное значение

for (port=0;port<6;port++){
if (PWM_width_set[port]<PWM_width_now[port]){PORTD&= ~(1<<port);} //включаем порт, если now меньше set
if (PWM_width_set[port]>=PWM_width_now[port]){PORTD|=1<<port;}//выключаем
...
if (Width_C4<Width_C4_now){PORTC&= ~(1<<4);} //включаем порт, если now меньше set
if (Width_C4>Width_C4_now){PORTC|=1<<4;}//выключаем
