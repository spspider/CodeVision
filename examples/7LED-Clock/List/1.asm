...
; 0000 002B {
_Dig_init:
; 0000 002C   Dig[0] = 95;   // Сейчас у нас схема с общим катодом
	LDI  R30,LOW(95)
	STS  _Dig,R30
; 0000 002D   Dig[1] = 24;
	LDI  R30,LOW(24)
...
; 0000 0058 #asm("sei")
	sei
; 0000 0059 Dig_init(); //инициализация массива с двоичным кодом
	RCALL _Dig_init
; 0000 005A while (1)
_0x6:
; 0000 005B       {
; 0000 005C 
; 0000 005D 
; 0000 005E  if (PINC.0==0) // если нажата первая кнопка
	SBIS 0x13,0
; 0000 005F   {
; 0000 0060   hour++ ; // к значению часы добавляем единицу
	INC  R4
; 0000 0061   }
; 0000 0062   if (PINC.1==0) // если нажата вторая кнопка
	SBIS 0x13,1
; 0000 0063   {
; 0000 0064   hour--; // к значению часы вычитаем единицу у
	DEC  R4
; 0000 0065   }
; 0000 0066   if (PINC.2==0) // если нажата третья кнопка
	SBIS 0x13,2
; 0000 0067   {
; 0000 0068   min++; // к значению минуты добавляем единицу
	INC  R5
; 0000 0069   }
; 0000 006A   if (PINC.3==0) // если нажата четвертая кнопка
	SBIS 0x13,3
; 0000 006B   {
; 0000 006C   min--; // к значению минуты вычитаем единицу
	DEC  R5
; 0000 006D   }
; 0000 006E   if (PINC.4==0) // кнопка RESET
; 0000 006F   {
; 0000 0070   }
; 0000 0071          // защита переполнения переменной, она нужна для того чтобы в моментах когда время не переходило в минус.
...
; 0000 0075   min=0;
	CLR  R5
; 0000 0076          // выводим переменные (стробирование)
; 0000 0077         Display(hour); //разложили "часы" на 2 цифры и отобразили по очереди
_0xF:
	MOV  R30,R4
	RCALL SUBOPT_0x2
; 0000 0078         PORTB=254;    //даем лог 0 для катода 1 разряда
	LDI  R30,LOW(254)
	RCALL SUBOPT_0x3
; 0000 0079         PORTD=Disp6;  //1 цифра
; 0000 007A         delay_ms(5);
; 0000 007B         PORTB=253;    //даем лог 0 для катода 2 разряда
	LDI  R30,LOW(253)
	RCALL SUBOPT_0x4
; 0000 007C         PORTD=Disp7;   //2 цифра
; 0000 007D         delay_ms(5);
; 0000 007E         Display(min);  //разложили "минуты" на 2 цифры и отобразили по очереди
	MOV  R30,R5
	RCALL SUBOPT_0x2
; 0000 007F         PORTB=251;      //даем лог 0 для катода 3 разряда
	LDI  R30,LOW(251)
	RCALL SUBOPT_0x3
; 0000 0080         PORTD=Disp6;  //3 цифра
; 0000 0081         delay_ms(5);
; 0000 0082         PORTB=247;    //даем лог 0 для катода 4 разряда
	LDI  R30,LOW(247)
	RCALL SUBOPT_0x4
