...
static void convertUniToAscii(char *buffer) {
 CHAR buf2[64];
 WideCharToMultiByte(CP_UTF8,0,(PWSTR)buffer,-1,buf2,64,NULL,NULL);
 lstrcpy(buffer,buf2);
}
...
/* attempt opening for R/W -- we don't care about devices which can't be accessed */
  handle = CreateFile(deviceDetails->DevicePath, GENERIC_READ|GENERIC_WRITE,
    FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING, openFlag, NULL);
  if(handle == INVALID_HANDLE_VALUE){
...
/* errorCode = USB_ERROR_ACCESS; opening will always fail for mouse -- ignore */
  ...
  if(vendorName != NULL && productName != NULL){
   ...
   if(strcmp(vendorName, buffer) != 0) continue;
   ...
   if(strcmp(productName, buffer) != 0) continue;
  }
  break;  /* we have found the device we are looking for! */
 }
 SetupDiDestroyDeviceInfoList(deviceInfoList);
 if (deviceDetails) free(deviceDetails);
 if(handle != INVALID_HANDLE_VALUE){
