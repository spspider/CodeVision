...
// pompage total, trouve dans les libs de sdcc
...
// _buffer - буфер, куда будет помещен результат
// _value - число (мксимум 65536)
// _n - количество 10 тичных разрядов.
///////////////////////////////////////////
void uintToStr(uint8_t* _buffer, uint16_t _value, uint8_t _n) {
  uint8_t i,j;
  uint16_t num;
...
uint8_t asctime (uint32_t local_time, uint8_t utc, uint8_t sw_time, uint8_t* _buffer) {
char str[6];
time_t s_time;
struct tm *date_time;

	s_time = local_time-2208988800; // Коректировка до 1970г.
	s_time = s_time + utc*3600; // Корректировка часового пояса
	date_time = gmtime(&s_time);


	uintToStr(str, lastTime.tm_mday, 2);
	_buffer[0] = str[0];
	_buffer[1] = str[1];
	_buffer[2] = ' ';
	
	_buffer[3] = __month[lastTime.tm_mon][0];
	_buffer[4] = __month[lastTime.tm_mon][1];
	_buffer[5] = __month[lastTime.tm_mon][2];

	_buffer[6] = ' ';
	
	uintToStr(str, lastTime.tm_hour, 2);
	_buffer[7] = str[0];
	_buffer[8] = str[1];
	_buffer[9] = ':';

	uintToStr(str, lastTime.tm_min, 2);
	_buffer[10] = str[0];
	_buffer[11] = str[1];
	_buffer[12] = ':';

	uintToStr(str, lastTime.tm_sec, 2);
	_buffer[13] = str[0];
	_buffer[14] = str[1];
